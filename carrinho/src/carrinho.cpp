...
void Carrinho::logStatus(float erro, float omega)
{
    ...
    if (agora - tPrevLog < LOG_MS)
        return;
    tPrevLog = agora;

    const char *est =
        (estado == CALIBRACAO) ? "CALIBRACAO" : (estado == ESPERANDO_LARGADA) ? "ESPERANDO_LARGADA"
                                                                              : "CORRIDA";

    auto finitesafe = [](float x) -> float
        ...
        if (c == '\n' || c == '\r')
        {
            ...
            else if (cmdBuf[0] == 'v' && cmdBuf[1] == 'y')
            {
                ...
                if (parseFloatAfter(cmdBuf, 2, v))
                {
                    ...
                    Serial.println("vy invalido");
            }
            else if (cmdBuf[0] == 'k' && cmdBuf[1] == 'p')
            {
                float v;
                if (parseFloatAfter(cmdBuf, 2, v) && v > 0.0f)
                {
                    kp = v;
                    Serial.printf("kp = %.3f\n", kp);
                }
                else
                    Serial.println("kp invalido");
            }
            ...
            else if (cmdBuf[0] == 'o' && cmdBuf[1] == 'm')
            {
                ...
                {
                    omegaMax = v;
                    Serial.printf("omegaMax = %.1f\n", omegaMax);
                }
                else
                    Serial.println("omegaMax invalido");
            }
            else
            {
...
void Carrinho::acionaRodasOminiInt(int vx, int vy, int omega)
{
    ...
    auto iround = [](int x) -> int
    { return (x >= 0 ? (x + 50) / 100 : (x - 50) / 100); };
...
void Carrinho::tick()
{
    ...
    {
    case CALIBRACAO:
        modoCalibracao(mascara, erro);
        break;
    case ESPERANDO_LARGADA:
        modoEsperandoLargada(mascara);
        break;
    case CORRIDA:
        modoCorrida(mascara, erro, dt);
        break;
    }
}
